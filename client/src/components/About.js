...
const About = () => {
    ...
    <Wrapper>
        ...
        <BioDiv><p>Earth Trotter is Xarah Dion's final project for the Concordia University Web Development Bootcamp. A full-stack MERN app making use of HTML, CSS, ES6 JavasScript and React on the frontend, NodeJS, ExpressJS and MongoDB on the backend. Its main functionality is to display logged travels as markers on an interactive world map with the help of MapboxGL API. </p>
                <p>Coordinates are generated in the backend with OpenCage API and stored in MongoDB along with the travel date, city, country and an optional field for location and/or details. The travels are displayed conditionally on the world map according to the year selected in the header in a dropdown menu. A city page is accessed by clicking the marker popups, where an image, “known for” tags and coordinates are displayed. These resources are retrieved from RoadGoat API and tested before rendering. For example, if there are no images in the database for the chosen city, the results fall back to the country image. The city page also presents a current weather widget with icon, temperatures and other parameters collected from OpenWeather API.</p>
